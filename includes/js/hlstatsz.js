    ...
    loadTabContent(pages, targetId, clickedTab) {
        ...
        targetDiv.addEventListener('fetch:loaded', function(e) {
            ...
            if (!tmp.textContent.trim()) {
                targetDiv.innerHTML = `<p class="hlstats-no-data center"><em>${t('not.enough.data')}</em></p>`;
            }
        }, { once: true });
...
function Location(country='', state='', city='')
{
    ...
    return loc.length ? loc.join(', ') : t('unknown');
}
  ...
  return `
    <div class="hlstats-map-card">
      ...
      <div class="hlstats-map-row">
        <span class="hlstats-map-label">${t('th.server')}</span>
        <a class="hlstats-map-value" href="steam://connect/${encodeURI(addrRaw)}">${addrText}</a>
      </div>

      <div class="hlstats-map-row">
        <span class="hlstats-map-label">${t('players')}</span><span class="hlstats-map-value">${server.act_players}/${server.max_players}</span>
      </div>

      <div class="hlstats-map-row">
        <span class="hlstats-map-label">${t('th.map')}</span><span class="hlstats-map-value">${server.act_map}</span>
      </div>

      <div class="hlstats-map-actions">
        <a class="hlstats-map-ghost" href="steam://connect/${encodeURI(addrRaw)}"><button>${t('map.join')}</button></a>
        ${DetailPage ? `
        <a class="hlstats-map-ghost" href="hlstats.php?game=${encodeURIComponent(server.game)}"><button>${t('map.details')}</button></a>` : ``}
      </div>
    </div>
  `;
}
...
function playerCard(player) {
  ...
  return `
    <div class="hlstats-map-card">
      ...
      <div class="hlstats-map-row">
        <span class="hlstats-map-label">${t('map.connected')}</span> <span class="hlstats-map-value">${connected}</span>
      </div>

      <div class="hlstats-map-row">
        <span class="hlstats-map-label">${t('team')}</span> <span class="hlstats-map-value">${team}</span>
      </div>

      <div class="hlstats-map-row">
        <span class="hlstats-map-label">${t('th.kd')}</span> <span class="hlstats-map-value">${player.kills}:${player.deaths}</span>
      </div>

      <div class="hlstats-map-actions">
        <a class="hlstats-map-ghost" href="hlstats.php?mode=playerinfo&player=${encodeURIComponent(player.playerId)}"><button>${t('map.profile')}</button></a>
      </div>
    </div>
  `;
}
 ...
 */
const Tooltip = (() => {
    let tip        = null;   // singleton DOM node
    let current    = null;   // element currently showing for
    let hideTimer  = null;
    let touchTimer = null;   // auto-close after touch
    ...
    function show(anchor) {
        ...
        const t = getEl();
        t.textContent = text;
        // reset so transition plays from scratch each time
        t.classList.remove('visible', 'hlz-tip--below');
        void t.offsetWidth;
        place(anchor, t);
    ...
    function place(anchor, t) {
        ...
        const goBelow = r.top - th - gap < GAP_EDGE;
        const top  = goBelow ? r.bottom + gap : r.top - th - gap;
        const left = Math.max(GAP_EDGE, Math.min(
            r.left + r.width / 2 - tw / 2,
            vw - tw - GAP_EDGE
        ));

        // point the arrow toward the anchor, not just the tooltip centre
        const anchorCx  = r.left + r.width / 2;
        const arrowLeft = Math.max(12, Math.min(anchorCx - left, tw - 12));

        t.classList.toggle('hlz-tip--below', goBelow);
        t.style.top  = top  + 'px';
        t.style.left = left + 'px';
        t.style.setProperty('--arrow-x', arrowLeft + 'px');
    }
    ...
    // Touch: tap to show/hide, auto-close after 3 s
...
function formatNum(num) {
    ...
    if (absNum >= 1.0e9) {
        formatted = (num / 1.0e9).toFixed(1).replace(/\.0$/, '') + "B";
    } else if (absNum >= 1.0e6) {
        formatted = (num / 1.0e6).toFixed(1).replace(/\.0$/, '') + "M";
    } else if (absNum >= 1.0e5) {
        formatted = (num / 1.0e5).toFixed(1).replace(/\.0$/, '') + "K";
    } else {
