...
");
    ...
    if ($db->num_rows($result)) {
        if (empty($_GET['ajax'])) {
        printSectionTitle(t('title.player.kill.stats').$asterisk);
        ?>
<div id="playerkills">
...
<div  class="responsive-table">
  <table class="maps-table">
    <tr>
        <th class="hlstats-ranking nowrap<?= isSorted('rank_position',$sort,$sortorder) ?>" style="width:1%"><?= headerUrl('rank_position', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.rank') ?></a></th>
        <th class="hlstats-main-description left<?= isSorted('name',$sort,$sortorder) ?>"><?= headerUrl('name', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('name') ?></a></th>
        <th class="<?= isSorted('kills',$sort,$sortorder) ?>"><?= headerUrl('kills', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.kills') ?></a></th>
        <th class="hide-2 meter-ratio <?= isSorted('kpercent',$sort,$sortorder) ?>"><?= headerUrl('kpercent', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.ratio') ?></a></th>
        <th class="hide<?= isSorted('deaths',$sort,$sortorder) ?>"><?= headerUrl('deaths', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.deaths') ?></a></th>
        <th class="hide-2 meter-ratio <?= isSorted('dpercent',$sort,$sortorder) ?>"><?= headerUrl('dpercent', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.ratio') ?></a></th>
        <th class="hide-3<?= isSorted('kpd',$sort,$sortorder) ?>"><?= headerUrl('kpd', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.kd') ?></a></th>
        <th class="hide-1<?= isSorted('headshots',$sort,$sortorder) ?>"><?= headerUrl('headshots', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.headshots') ?></a></th>
        <th class="hide-2 meter-ratio <?= isSorted('hpercent',$sort,$sortorder) ?>"><?= headerUrl('hpercent', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.ratio') ?></a></th>
        <th class="hide-3<?= isSorted('hpk',$sort,$sortorder) ?>"><?= headerUrl('hpk', ['playerkills_sort','playerkills_sortorder'], 'playerkills').t('th.hsk') ?></a></th>
    </tr>
    <?php
        ...
        while ($res = $db->fetch_array($result))
        {
            ...
            if ($g_options['countrydata']) {
             ...
             echo '<a href="?mode=playerinfo&amp;player='.$res['victimId'].'"><span class="hlstats-name">'.htmlspecialchars(html_entity_decode($res['name'], ENT_QUOTES | ENT_HTML5, 'UTF-8'), ENT_COMPAT).'&nbsp;</span></a></td>
