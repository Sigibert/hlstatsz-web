...
printSectionTitle(t('title.roles.stats'));
?>
<div class="hlstats-cards-grid">
<section class="hlstats-section hlstats-card">
  <div class="hlstats-card-foot"><?= t('roles.stats', ["{kills}" => '<strong>'. nf($realkills) . '</strong>', "{deaths}" => '<strong>'.nf($realdeaths).'</strong>']) ?></div>
</section>
</div>
...
<div class="responsive-table">
  <table class="roles-table">
    <tr>
        <th class="hlstats-ranking nowrap right<?= isSorted('rank_position',$sort,$sortorder) ?>"><?= headerUrl('rank_position', ['sort','sortorder'], 'roles') .t('th.rank') ?></a></th>
        <th class="hlstats-main-column left<?= isSorted('code',$sort,$sortorder) ?>"><?= headerUrl('code', ['sort','sortorder'], 'roles') .t('th.role') ?></a></th>
        <th class="nowrap<?= isSorted('picked',$sort,$sortorder) ?>"><?= headerUrl('picked', ['sort','sortorder'], 'roles') .t('th.picked') ?></a></th>
        <th class="meter-ratio nowrap hide-2<?= isSorted('ppercent',$sort,$sortorder) ?>"><?= headerUrl('ppercent', ['sort','sortorder'], 'roles') ?>Ratio</a></th>
        <th class="nowrap hide<?= isSorted('kills',$sort,$sortorder) ?>"><?= headerUrl('kills', ['sort','sortorder'], 'roles') .t('th.kills') ?></a></th>
        <th class="meter-ratio nowrap hide-2<?= isSorted('kpercent',$sort,$sortorder) ?>"><?= headerUrl('kpercent', ['sort','sortorder'], 'roles') .t('th.ratio') ?></a></th>
        <th class="nowrap hide<?= isSorted('deaths',$sort,$sortorder) ?>"><?= headerUrl('deaths', ['sort','sortorder'], 'roles') .t('th.deaths') ?></a></th>
        <th class="meter-ratio nowrap hide-2<?= isSorted('dpercent',$sort,$sortorder) ?>"><?= headerUrl('dpercent', ['sort','sortorder'], 'roles') .t('th.ratio') ?></a></th>
        <th class="nowrap hide-3<?= isSorted('kpd',$sort,$sortorder) ?>"><?= headerUrl('kpd', ['sort','sortorder'], 'roles') .t('th.kd') ?></a></th>
    </tr>
    <?php
