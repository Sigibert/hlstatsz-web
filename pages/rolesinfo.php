...
if ( !defined('IN_HLSTATS') ) { die('Do not access this file directly'); }
	...
	$role = valid_request($_GET['role'] ?? '', false) or error(t('error.no.uniqueid'));
...
if (!is_ajax()){
printSectionTitle(t('title.roles.details'));
...
echo '<div class="hlstats-cards-grid">
  <section class="hlstats-section hlstats-card">';
    ...
    echo '<div class="hlstats-award'.($numitems>0?' has-winner':'').'">
            ...
            <div class="hlstats-award-count">'.t('total.role.stats',
               ["{kills}" => '<strong>'.nf($totalkills).'</strong>',
               "{role}"   => '<strong>'.htmlspecialchars($role_name).'</strong>']).'</div>
          </div>
  </section>
</div>';
}
...
<div class="responsive-table">
  <table class="players-table">
    <tr>
        <th class="hlstats-ranking nowrap<?= isSorted('rank_position',$sort,$sortorder) ?>"><?= headerUrl('rank_position', ['sort','sortorder'], 'rolesinfo') .t('th.rank') ?></a></th>
        <th class="hlstats-main-description left<?= isSorted('killerName',$sort,$sortorder) ?>"><?= headerUrl('killerName', ['sort','sortorder'], 'rolesinfo') .t('player') ?></a></th>
        <th class="<?= isSorted('frags',$sort,$sortorder) ?>"><?= headerUrl('frags', ['sort','sortorder'], 'rolesinfo') . ucfirst($role_name)."&nbsp;".t('th.kills') ?></a></th>
    </tr>
    <?php
...
?>
<div class="hlstats-note"><a href="?mode=roles&amp;game=<?= $game ?>&amp;tab=ranks">&larr;&nbsp;<?= t('goto.role.stats') ?></a></div>
