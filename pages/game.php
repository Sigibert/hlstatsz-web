...
?>
		  <table class="livestats-table-server hlstats-table-fixed<?= $i == 0 ? ' livestats-table-first':' livestats-table-next' ?>">
				<thead><tr>
					<th class="hlstats-main-server left responsive"><?= t('th.server') ?></th>
...
<?php } else { echo '<table class="livestats-table-server hlstats-table-fixed '.($i==count($servers)-1? ' livestats-table-last':' livestats-table-next').'"><tbody>'; } ?>
			...
			if ($slider) echo '<span class="hlstats-slider-arrow" data-tooltip="'.t('click.detail').'">▼</span>';
			...
			echo "\" alt=\"$game\" /></span>";
			echo "<a href=\"" . $g_options['scripturl'] . "?mode=servers&amp;server_id=$server_id\"><span class=\"hlstats-name\" data-tooltip=\"".t('title.server.live')."\">" . htmlspecialchars(html_entity_decode($rowdata['name'], ENT_QUOTES | ENT_HTML5, 'UTF-8'), ENT_COMPAT) . "</span></a>";
	?></td>
			  ...
			  <td style="padding:0px;text-align:center;" colspan="8">
				...
				<a href="<?= $g_options['scripturl'] ?>?mode=servers&amp;server_id=<?= (int)$server_id ?>&amp;game=<?= htmlspecialchars($game, ENT_QUOTES) ?>"
				   style="text-decoration:none;" data-tooltip="<?= t('view.graphs') ?>">
				    ...
				    <span class="show-2"><?= t('view.graphs') ?></span>
				</a>
				<?php } ?>
			  </td>
			</tr>
...
(function() {
    ...
    document.querySelectorAll('.hlstats-server-row').forEach(function(row) {
        ...
        row.dataset.tooltip = t('click.detail');
...
<?php
	printSectionTitle(t('title.daily.awards').' - '.formatDate(strtotime($awards_d_date),IntlDateFormatter::FULL, IntlDateFormatter::NONE));
?>
		<table>
	...
	<td class="left"><?php
				...
				if ($awarddata['d_winner_id']) {
                    ...
                    if ($g_options['countrydata']) {
                        echo "<span class=\"hlstats-flag\"><img src=\"".getFlag($awarddata['flag'])."\" alt=\"".$awarddata['country']."\" data-tooltip=\"".htmlspecialchars($awarddata['country'], ENT_QUOTES)."\" /></span>";
                    }
...
<?php
            ...
            if (!$c) {
                echo '<tr><td class="left"><em>'.t('no.winner').'</em></td></tr>';
            }
?></table>
