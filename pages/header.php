...
if ( !defined('IN_HLSTATS') ) { die('Do not access this file directly'); }
    ...
    // Language selection
    $langOptions = [];
    if (!empty($g_options['Language'])) {
        $langOptions = array_values(array_filter(
            array_slice(array_map('trim', explode(';', $g_options['Language'])), 0, 15),
    ...
    if (!empty($langOptions)) {
        if (!empty($_GET['lang'])) {
            ...
            if (in_array($newLang, $langOptions)) {
                myCookie('lang', $newLang, time()+60*60*24*30);
                $_COOKIE['lang'] = $newLang;
            }
            $query = updateQueryKey(["lang" => '']);
            $q = $query ? '?' : '';
            header('Location: ' . $_SERVER['PHP_SELF'] . $q . $query);
            exit();
        }
        $cookieLang = isset($_COOKIE['lang']) ? preg_replace('/[^a-z]/', '', $_COOKIE['lang']) : '';
        $activeLang = in_array($cookieLang, $langOptions) ? $cookieLang : $langOptions[0];
        $_COOKIE['lang'] = $activeLang;
    }
?>
<!DOCTYPE html>
<head>
    ...
    <?php if (isset($g_options['chart']) && $g_options['chart'] != 0) { ?>
  ...
  <div class="hlstats-top">
    <div class="hlstats-inner">
      ...
      <nav class="hlstats-topnav">
        <a class="hlstats-navlink<?= active(['contents'], $currentMode) ?>" href="<?= $g_options['scripturl'] ?>"><?= t('nav.contents') ?></a>
        <?php
        if ($g_options['sourcebans_address']) {
          echo '<a class="hlstats-navlink" href="'. $g_options['sourcebans_address'] . '" target="_blank">' . t('nav.sourcebans') . '</a>';
        }
        if ($g_options['forum_address']) {
          echo '<a class="hlstats-navlink" href="' . $g_options['forum_address'] . '">' . t('nav.forum') . '</a>';
        }
        ?>
        <a class="hlstats-navlink<?= active(['help'], $currentMode) ?>" href="<?= $g_options['scripturl'] ?>?mode=help"><?= t('nav.help') ?></a>
      </nav>
      <div class="hlstats-topright">
        <?php if (count($langOptions) > 1) { ?>
        <div class="hlstats-lang">
          <?php foreach ($langOptions as $lc) { ?>
          <a href="<?= htmlspecialchars($_SERVER['PHP_SELF'] . '?' . updateQueryKey(["lang" => $lc])) ?>"
             class="hlstats-lang-flag<?= $lc === $activeLang ? ' active' : '' ?>"
             data-tooltip="<?= htmlspecialchars(strtoupper($lc)) ?>">
            <img src="<?= getFlag($lc, 'url') ?>" alt="<?= htmlspecialchars($lc) ?>">
          </a>
          <?php } ?>
        </div>
        <?php } ?>
        <div class="hlstats-login nowrap"><?= $signin ?></div>
      </div>
    </div>
  </div>
...
<?php
    ...
    if ($g_options['display_gamelist'] == 1 && !empty($game) && $mode != 'admin' && $mode != 'help') {
        ...
        $modes = array('players','clans','awards','chat','actions','weapons','maps','bans','countryclans');
        while ($gamedata = $db->fetch_row($resultGames)) {
            if ($gamedata[0] == $game) continue;
            $query = in_array($mode, $modes) && !empty($_GET['game']) ?
                     updateQueryKey(["game" => $gamedata[0]]) : 'game='.$gamedata[0];
            $gamelist .= '<li><a href="?'.$query.'">'.$gamedata[1].'</a></li>';
        }
    }
...
?>
      <nav class="hlstats-subnav">
        <div class="hlstats-dropdown hlstats-game-dropdown<?php echo !empty($gamelist) ? ' has-games' : ''; ?>" id="game-dropdown">
          ...
          <div class="hlstats-dropmenu">
            <a href="?game=<?= $game ?>" class="hlstats-server"><?= $gameName ?></a>
            <ul id="gamelist-menu"><?= $gamelist ?></ul>
          </div>
        </div>
        <a href="?mode=players&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['players','playerinfo','playerhistory','playersessions','playerawards','chathistory'], $mode) ?>"><?= t('sub.nav.players') ?></a>
            <a href="?mode=clans&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['clans','claninfo'], $mode) ?>"><?= t('sub.nav.clans') ?></a>
        <a href="?mode=awards&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['awards','rankinfo','ribboninfo','dailyawardinfo'], $mode) ?>"><?= t('sub.nav.awards') ?></a>
        ...
        <div class="hlstats-dropdown">
          <button class="hlstats-dropbtn"
                  ...
                  aria-haspopup="true"
                  aria-expanded="false">
            <?= t('sub.nav.sections') ?><span class="caret">&#9660;</span>
          </button>

          <div class="hlstats-dropmenu">
           <?php if ($g_options['nav_globalchat']==1) { ?>
              <a href="?mode=chat&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['chat'], $mode) ?>"><?= t('sub.nav.chat') ?></a>
            <?php } ?>
            <a href="?mode=actions&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['actions','actioninfo'], $mode) ?>"><?= t('sub.nav.actions') ?></a>
            <a href="?mode=weapons&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['weapons','weaponinfo'], $mode) ?>"><?= t('sub.nav.weapons') ?></a>
            <a href="?mode=maps&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['maps','mapinfo'], $mode) ?>"><?= t('sub.nav.maps') ?></a>
            <?php
            ...
            if ($numitems > 0) {
            ?>
                <a href="?mode=roles&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['roles','rolesinfo'], $mode) ?>"><?= t('sub.nav.roles') ?></a>
            <?php } ?>
            <?php if ($g_options['nav_cheaters']==1) { ?>
              <a href="?mode=bans&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['bans'], $mode) ?>"><?= t('sub.nav.bans') ?></a>
            <?php } ?>
            <?php if ($g_options['countrydata']) { ?>
              <a href="?mode=countryclans&amp;game=<?= $game ?>" class="hlstats-sublink<?= active(['countryclans','countryclansinfo'], $mode) ?>"><?= t('sub.nav.countries') ?></a>
            <?php } ?>
...
<?php if ($mode !== 'admin') { ?>
      <div class="hlstats-header-search" id="hlstats-search">
        <form class="hlstats-form" method="get" action="<?= $g_options['scripturl'] ?>">
          ...
          <input id="hlstatsSearchQ" type="text" name="q" placeholder="<?= t('search.find') ?>" autocomplete="off" />
