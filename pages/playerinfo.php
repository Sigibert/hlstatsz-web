...
function makeClanTag(string $name): array{
    ...
    if (!$player && $uniqueid) {
        ...
        if ($db->num_rows() > 1) {
            ...
            error(t('error.uniqueid',["{uniqueid}" => $uniqueid]));
        } else {
    ...
    if ($hideranking == 2) {
        $statusmsg = '<span class="hlstats-status banned;">'.t('karma.banned').'</span>';
    } else {
...
<ul class="hlstats-tabs" id="tabs_playerinfo">
    <li class="active">
        <a href="#general_aliases" class="tab" data-url="general_aliases" data-target="tab1"><?= t('player.tab1') ?></a>
    </li>
    <li>
        <a href="#playeractions_teams" class="tab" data-url="playeractions_teams" data-target="tab2"><?= t('player.tab2') ?></a>
    </li>
    <li>
        <a href="#weapons" class="tab" data-url="weapons" data-target="tab3"><?= t('player.tab3') ?></a>
    </li>
    <li>
        <a href="#mapperformance_servers" class="tab" data-url="mapperformance_servers" data-target="tab4"><?= t('player.tab4') ?></a>
    </li>
    <li>
        <a href="#killstats" class="tab" data-url="killstats" data-target="tab5"><?= t('player.tab5') ?></a>
    </li>
</ul>
</div>
