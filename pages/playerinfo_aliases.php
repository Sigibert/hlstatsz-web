...
if ( !defined('IN_HLSTATS') ) { die('Do not access this file directly'); }
	...
	if ($numitems > 1)
	{
       ...
       if (empty($_GET['ajax'])) {
		...
		printSectionTitle(t('title.aliases'));
?>
<div id="aliases">
<?php
}
...
<div  class="responsive-table">
  <table class="players-table">
    <tr>
        ...
        <th class="hlstats-main-description left<?= isSorted('name',$sort,$sortorder) ?>"><?= headerUrl('name', ['aliases_sort','aliases_sortorder'], 'aliases') .t('name') ?></a></th>
        <th class="<?= isSorted('lastuse',$sort,$sortorder) ?>"><?= headerUrl('lastuse', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.last.use') ?></a></th>
        <th class="<?= isSorted('kills',$sort,$sortorder) ?>"><?= headerUrl('kills', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.kills') ?></a></th>
        <th class="hide<?= isSorted('deaths',$sort,$sortorder) ?>"><?= headerUrl('deaths', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.deaths') ?></a></th>
        <th class="hide-1<?= isSorted('kpd',$sort,$sortorder) ?>"><?= headerUrl('kpd', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.kd') ?></a></th>
        <th class="hide<?= isSorted('headshots',$sort,$sortorder) ?>"><?= headerUrl('headshots', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.headshots') ?></a></th>
        <th class="hide-1<?= isSorted('hpk',$sort,$sortorder) ?>"><?= headerUrl('hpk', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.hsk') ?></a></th>
        <th class="hide-1<?= isSorted('suicides',$sort,$sortorder) ?>"><?= headerUrl('suicides', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.suicides') ?></a></th>
       <th class="hide-2<?= isSorted('acc',$sort,$sortorder) ?>"><?= headerUrl('acc', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.accuracy') ?></a></th>
       <th class="hide-2<?= isSorted('connection_time',$sort,$sortorder) ?>"><?= headerUrl('connection_time', ['aliases_sort','aliases_sortorder'], 'aliases') .t('th.time')?></a></th>
   </tr>
    <?php
        $i= 1 + $start;
