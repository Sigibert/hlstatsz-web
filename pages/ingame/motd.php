...
if ( !defined('IN_HLSTATS') ) { die('Do not access this file directly'); }
	...
	if (isset($_GET['players']) && is_numeric($_GET['players'])) {
		$players = min(100,valid_request($_GET['players'], true)); // safe guard max Top 100
	}

	$clans = 5;  
	if (isset($_GET['clans']) && is_numeric($_GET['clans'])) {
		$clans = min(100,valid_request($_GET['clans'], true));
	}

	$servers = 5;  
	if (isset($_GET['servers']) && is_numeric($_GET['servers'])) {
		$servers = min(100,valid_request($_GET['servers'], true));
	}
...
{
    global $game, $g_options, $sort, $sortorder, $rank_type, $players;
...
printSectionTitle("Players - Top $players");
?>
...
function qClansRank()
{
    global $game, $g_options, $sort, $sortorder, $rank_type, $clans;
    ...
    printSectionTitle("Clans - Top $clans");
?>
...
<?php
    $query= "
            ...
            WHERE
                game='$game'
            ORDER BY
                sortorder, act_players, name, serverId
            LIMIT $servers OFFSET 0
    ";
	$db->query($query);
	$this_server = array();
	$servers = array();
