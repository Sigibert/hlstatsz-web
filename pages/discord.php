...
include (PAGE_PATH . '/voicecomm_serverlist.php');
	...
	if ($widget && !isset($widget['code'])) {
		...
		$outp_str = show("discord", array(
			"head"           => t('title.discord.overview'),
			"server_name"    => t('voice.server.name'),
			"members_online" => t('discord.members.online'),
			"voice_channels" => t('discord.voice.channels'),
			"invite_link"    => t('discord.invite.link'),
			"name"           => $name,
			"presence"       => $presence,
			"channels_count" => count($channels),
			"invite"         => $invite_html,
			"channel_head"   =>  t('title.discord.voice'),
			"uchannels"      => $channel_html,
			"members_head"   => t('title.discord.members'),
