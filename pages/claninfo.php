...
if ( !defined('IN_HLSTATS') ) { die('Do not access this file directly'); }
	...
	$clan = valid_request(intval($_GET["clan"] ?? 0), true) or error(t('error.no.uniqueid'));
	...
	$db->query("
		SELECT
			...
			MAX(hlstats_Players.activity) as activity,
			MAX(hlstats_Players.last_event) as last_event
		FROM
			hlstats_Clans
		LEFT JOIN
			hlstats_Players
	...
	if ($db->num_rows() != 1) {
		error(t('error.uniqueid',["{uniqueid}" => $clan]));
	}
...
<ul class="hlstats-tabs" id="tabs_playerinfo">
    <li class="active">
        <a href="#General" class="tab" data-url="general" data-target="tab1"><?= t('clan.tab1') ?></a>
    </li>
    <li>
        <a href="#actions_teams" class="tab" data-url="actions_teams" data-target="tab2"><?= t('clan.tab2') ?></a>
    </li>
    <li>
        <a href="#weapons" class="tab" data-url="weapons" data-target="tab3"><?= t('clan.tab3') ?></a>
    </li>
    <li>
        <a href="#mapperformance" class="tab" data-url="mapperformance" data-target="tab4"><?= t('clan.tab4') ?></a>
    </li>
</ul>
</div>
...
<div class="hlstats-note">
    <a href="?mode=clans&amp;game=<?= $game ?>">&larr;&nbsp;<?= t('goto.clan.rankings') ?></a>
</div>
