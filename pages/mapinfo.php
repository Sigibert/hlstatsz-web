...
if ( !defined('IN_HLSTATS') ) { die('Do not access this file directly'); }
	...
	$map = valid_request($_GET['map'] ?? '', false) or error(t('error.no.uniqueid'));
...
?>
	...
	<?php printSectionTitle(t('title.map.details')); ?>
<div class="hlstats-cards-grid">
<section class="hlstats-section hlstats-card">
  <div class="hlstats-pname"><?= $map ?></div>
  <div class="hlstats-card-foot"><?= t('map.stats', ["{kills}" => '<strong>'.nf(intval($totalkills)).'</strong>']) ?></div>
</section>
...
<?php // figure out URL and absolute path of image
	...
	if ($mapimg)
	{
		echo "<a href=\"" . $mapimg . "\" rel=\"boxed\" data-tooltip=\"".htmlspecialchars($map, ENT_QUOTES)."\"><img src=\"$mapimg\" alt=\"$map\" style=\"max-width:100%;\"/></a>";
	}
...
<div  class="responsive-table">
  <table class="maps-table">
    <tr>
        <th class="hlstats-ranking nowrap<?= isSorted('rank_position',$sort,$sortorder) ?>"><?= headerUrl('rank_position', ['sort','sortorder'], 'mapinfo') .t('th.rank') ?></a></th>
        <th class="hlstats-main-description left<?= isSorted('killerName',$sort,$sortorder) ?>"><?= headerUrl('killerName', ['sort','sortorder'], 'mapinfo') .t('player') ?></a></th>
        <th class="<?= isSorted('frags',$sort,$sortorder) ?>"><?= headerUrl('frags', ['sort','sortorder'], 'mapinfo') .t('th.kills') ?></a></th>
        <th class="hide<?= isSorted('headshots',$sort,$sortorder) ?>"><?= headerUrl('headshots', ['sort','sortorder'], 'mapinfo') .t('th.headshots') ?></a></th>
        <th class="hide-1<?= isSorted('hpk',$sort,$sortorder) ?>"><?= headerUrl('hpk', ['sort','sortorder'], 'mapinfo') .t('th.hsk') ?></a></th>
    </tr>
    <?php
...
<div class="hlstats-note">
    <a href="<?php echo $g_options['scripturl'] . "?mode=maps&amp;game=$game"; ?>">&larr;&nbsp;<?= t('goto.map.stats') ?></a>
</div>
<script>
Fetch.ini('mapinfo');
</script>
