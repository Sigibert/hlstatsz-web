    ...
    $result = $db->query("
        WITH frag_data AS (
            SELECT
                ...
                COUNT(*) AS kills,
                ROUND(COUNT(*) / $realkills * 100, 2) AS kpercent,
                SUM(f.headshot = 1) AS headshots,
                ROUND(SUM(f.headshot = 1) / IF(COUNT(*) = 0, 1, COUNT(*)), 2) AS hpk,
...
");
	...
	printSectionTitle(t('title.weapon.usage').$asterisk);
...
<div  class="responsive-table">
  <table class="weapons-table">
    <tr>
        <th class="hlstats-ranking nowrap<?= isSorted('rank_position',$sort,$sortorder) ?>"><?= headerUrl('rank_position', ['weap_sort','weap_sortorder'], 'weapons') .t('th.rank') ?></a></th>
        <th class="hlstats-main-column left<?= isSorted('weapon',$sort,$sortorder) ?>"><?= headerUrl('weapon', ['weap_sort','weap_sortorder'], 'weapons') .t('th.weapons') ?></a></th>
        <th class="hide-2<?= isSorted('modifier',$sort,$sortorder) ?>"><?= headerUrl('modifier', ['weap_sort','weap_sortorder'], 'weapons') .t('modifier') ?></a></th>
        <th class="<?= isSorted('kills',$sort,$sortorder) ?>"><?= headerUrl('kills', ['weap_sort','weap_sortorder'], 'weapons') .t('th.kills') ?></a></th>
        <th class="hide-2 meter-ratio <?= isSorted('kpercent',$sort,$sortorder) ?>"><?= headerUrl('kpercent', ['weap_sort','weap_sortorder'], 'weapons') .t('th.ratio') ?></a></th>
        <th class="hide<?= isSorted('headshots',$sort,$sortorder) ?>"><?= headerUrl('headshots', ['weap_sort','weap_sortorder'], 'weapons') .t('th.headshots') ?></a></th>
        <th class="hide-2 meter-ratio <?= isSorted('hpercent',$sort,$sortorder) ?>"><?= headerUrl('hpercent', ['weap_sort','weap_sortorder'], 'weapons') .t('th.ratio') ?></a></th>
        <th class="hide-3<?= isSorted('hpk',$sort,$sortorder) ?>"><?= headerUrl('hpk', ['weap_sort','weap_sortorder'], 'weapons') .t('th.hsk') ?></a></th>
    </tr>
    <?php
        while ($res = $db->fetch_array($result))
        {
            ...
            if ($image) {
                $weapimg = '<span class="hlstats-image"><img src="' . $image['url'] . '" alt="' . $fname[$weapon] . '" data-tooltip="'.htmlspecialchars($fname[$weapon], ENT_QUOTES) . '"></span><span class="hlstats-name">' . ((!empty($fname[$weapon])) ? $fname[$weapon] : ucwords(preg_replace('/_/', ' ', $weapon))) . '</span>';
            ...
            echo '<tr>
                  ...
                  <td class="nowrap hide-2">'.$res['modifier'].' times</td>
...
if (empty($_GET['ajax']) || $_GET['ajax'] == 'statsme') {
    ...
    if ($db->num_rows($result)) {
        if (empty($_GET['ajax'])) {
            printSectionTitle(t('title.weapon.stats').$asterisk);
?>
<div id="statsme">
<?php
}
?>
  ...
  <table class="statsme-table">
    <tr>
        <th class="hlstats-ranking nowrap<?= isSorted('rank_position',$sort,$sortorder) ?>"><?= headerUrl('rank_position', ['sm_sort','sm_sortorder'], 'statsme') .t('th.rank') ?></a></th>
        <th class="hlstats-main-description left<?= isSorted('smweapon',$sort,$sortorder) ?>"><?= headerUrl('smweapon', ['sm_sort','sm_sortorder'], 'statsme') .t('th.weapon') ?></a></th>
        <th class="<?= isSorted('smshots',$sort,$sortorder) ?>"><?= headerUrl('smshots', ['sm_sort','sm_sortorder'], 'statsme') .t('th.shots') ?></a></th>
        <th class="<?= isSorted('smhits',$sort,$sortorder) ?>"><?= headerUrl('smhits', ['sm_sort','sm_sortorder'], 'statsme').t('th.hits')?></a></th>
        <th class="<?= isSorted('smdamage',$sort,$sortorder) ?>"><?= headerUrl('smdamage', ['sm_sort','sm_sortorder'], 'statsme') .t('th.damage') ?></a></th>
        <th class="<?= isSorted('smheadshots',$sort,$sortorder) ?>"><?= headerUrl('smheadshots', ['sm_sort','sm_sortorder'], 'statsme') .t('th.headshots') ?></a></th>
        <th class="<?= isSorted('smkills',$sort,$sortorder) ?>"><?= headerUrl('smkills', ['sm_sort','sm_sortorder'], 'statsme') .t('th.kills') ?></a></th>
        <th class="<?= isSorted('smkdr',$sort,$sortorder) ?>"><?= headerUrl('smkdr', ['sm_sort','sm_sortorder'], 'statsme') .t('th.kd') ?></a></th>
        <th class="<?= isSorted('smaccuracy',$sort,$sortorder) ?>"><?= headerUrl('smaccuracy', ['sm_sort','sm_sortorder'], 'statsme') .t('th.accuracy') ?></a></th>
        <th class="<?= isSorted('smdhr',$sort,$sortorder) ?>"><?= headerUrl('smdhr', ['sm_sort','sm_sortorder'], 'statsme') .t('th.damage.hit') ?></a></th>
        <th class="<?= isSorted('smspk',$sort,$sortorder) ?>"><?= headerUrl('smspk', ['sm_sort','sm_sortorder'], 'statsme') .t('th.shots.kills') ?></a></th>
    </tr>
    <?php
        while ($res = $db->fetch_array($result))
        {
            ...
            echo '<tr>
                  ...
                  <td class="nowrap">'.nf($res['smshots']).' times</td>
                  <td class="nowrap">'.nf($res['smhits']).'</td>
                  <td class="nowrap">'.nf($res['smdamage']).'</td>
                  <td class="nowrap">'.nf($res['smheadshots']).'</td>
                  <td class="nowrap">'.nf($res['smkills']).'</td>
...
if (empty($_GET['ajax']) || $_GET['ajax'] == 'statsme2') {
  ...
  $result = $db->query($query);    	
	...
	printSectionTitle(t('title.weapon.targets').$asterisk);
...
?>
    ...
    <table>
    <tr>
       <td><span class="hlstats-name"><?= t('targets') ?></span></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <a href="javascript:switch_weapon('total');">
          <span class="hldstats-name"><?= t('hitbox.all.weapons') ?></span>
        </a>
        </td>
    </tr>
    </table>
