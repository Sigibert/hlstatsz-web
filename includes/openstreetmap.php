...
*/
global $db, $g_options, $clandata, $clan, $mode, $server_id;
...
if ( $type ==  "main" ) {
    // Servers
    $filter  = $mode == 'servers' ? "serverId='$server_id'" : "game='$game'";
    $filter2 = $mode == 'servers' ? "s.serverId='$server_id'" : "s.game='$game'";
    if (empty($game) && $mode != 'servers') {
        $db->query("SELECT 
                        s.serverId,
                        s.game,
