...
if ($image) {
    $favweapon =  $weaponlink_open . "<img src=\"" . $image['url'] . "\" alt=\"".$weap_name."\" data-tooltip=\"".htmlspecialchars($weap_name, ENT_QUOTES)."\" />" . $weaponlink_close;
} else {
...
<section class="hlstats-section hlstats-card">
  <div class="hlstats-card-title"><?= t('player.profile') ?></div>
        ...
        <div class="hlstats-profile-head">
            ...
            <div class="hlstats-identity">
                ...
                <div class="hlstats-meta">
                    ...
                    <span><strong><?= t('status') ?></strong> <span class="hlstats-status"><?= t('loading') ?></span></span>
                    <span><strong><?= t('member.since') ?></strong> <span id="steam-member-since-<?= (int)$player ?>"><?= t('loading') ?></span></span>
                </div>
            </div>
        </div>
    </div>
  ...
  <div class="hlstats-card-body hlstats-card-grid">
      <div class="label"><?= t('karma') ?></div><div class="value<?php echo $hideranking != 2 ? ' green':' red';?>"><strong><?= $statusmsg ?></strong></div>
    ...
    <form method="post">
      ...
      <div class="label"><?= t('member.clan') ?></div><div class="value"><?= $clan ?><input class="hlstats-user" type="text" name="clan_name" value="<?= htmlspecialchars($playerdata['clan_name'] ?? '') ?>" placeholder="[TAG] Name"></div>
      <div class="label"><?= t('homepage') ?></div><div class="value"><?= $homepage ?><input class="hlstats-user" type="text" name="homepage" value="<?= htmlspecialchars($playerdata['homepage'] ?? '') ?>"></div>

      <div class="label"></div><div class="value"><input type="submit" value="<?= t('update') ?>"><?php if ($error) echo ' <span class="red">' . $error . '</span>'; ?></div>


      <?php else: ?>

      <div class="label"><?= t('member.clan') ?></div><div class="value"><?= $clan ?></div>

      <div class="label"><?= t('homepage') ?></div><div class="value"><?= $homepage ?></div>

      <?php endif; ?>
      <div class="label"><?= t('first.connect') ?></div>
      <div class="value"><?= $playerdata['createdate'] ? formatDate($playerdata['createdate'], IntlDateFormatter::MEDIUM, IntlDateFormatter::MEDIUM, 'D. M. jS, Y @ H:i:s') : t('unknown') ?></div>

      <div class="label"><?= t('last.connect') ?></div>
      <div class="value"><?= $playerdata['last_event'] ? formatDate($playerdata['last_event'], IntlDateFormatter::MEDIUM, IntlDateFormatter::MEDIUM, 'D. M. jS, Y @ H:i:s') : t('unknown') ?></div>

      <div class="label"><?= t('last.ping') ?></div>
      <div class="value">
        <?php
          $ping = (int)($playerdata['lastPing'] ?? 0);
          $latency = $ping ? (int)round($ping / 2) : 0;
          echo $ping ? t('last.ping.value',["{ping}" => $ping, "{latency}" => $latency]) : '-';
        ?>
      </div>
      ...
      <div class="label"><?= t('favorite.weapon') . $asterisk ?></div><div class="value"><?= $favweapon ?></div>
    </div>
  </form>
...
<?php } ?>
          ...
          <div class="label"><?= t('headshots') ?></div>
          <div class="value">
                  <?php
       ...
       <?php
        ...
        ?>
          ...
          <div class="label"><?= t('headshots.kill') ?></div>
              <div class="value">
                  <?php
                      $db->query("
                          SELECT
          ...
          <div class="label"><?= t('shots.kill') ?></div>
          <div class="value">
                  <?php
                      $db->query("
                          SELECT
          ...
          <div class="label"><?= t('longest.ks') ?></div>
         <div class="value">
                  <?php
          ...
          <div class="label"><?= t('longest.ds') ?></div>
          <div class="value">
                  <?php
      ...
      <div class="hlstats-card-foot hlstats-card-foot-margin">
          <?php
              echo '🗓️ <strong>'
                  . t('player.history', ["{name}" => htmlspecialchars(html_entity_decode($playerdata['lastName'], ENT_QUOTES | ENT_HTML5, 'UTF-8')) . '</strong>'])."&nbsp;&nbsp;";
              echo '<a href="' . $g_options['scripturl'] . "?mode=playerhistory&amp;player=$player\">".t('events')."</a> &nbsp;|&nbsp; ";
              echo '<a href="' . $g_options['scripturl'] . "?mode=playersessions&amp;player=$player\">".t('sessions')."</a> &nbsp;|&nbsp; ";
              ...
              echo "<a href=\"" . $g_options['scripturl'] . "?mode=playerawards&amp;player=$player\">".t('awards')."&nbsp;($numawards)</a>";
      
              if ($g_options["nav_globalchat"] == 1) {
                  echo " &nbsp;|&nbsp; <a href=\"" . $g_options['scripturl'] . "?mode=chathistory&amp;player=$player\">".t('chat')."</a>";
              }
...
<section class="hlstats-section hlstats-card">
            ...
            <div class="hlstats-award-icon">
                <?php echo '<img src="'.$rankimage['url']."\" alt=\"$rankName\"  data-tooltip=\"$rankName\" />"; ?>
            </div>
            </div>
...
<div class="hlstats-cards-grid">
  ...
  <section class="hlstats-section hlstats-card">
    ...
    <?php
        ...
        echo "<div class=\"hlstats-forum-signature\"><img src=\"$imglink\" alt=\"forum sig image\"/></div>";
